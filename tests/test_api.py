...
def test_add_group(slapd, config):
    ...
    slapd.add_group('testgroup', gid=50000, ldif_path=prefix + 'ldap-add-group.ldif', **config)
    assert len(slapd.get_group('testgroup')) == 1
...
def test_add_user(slapd, config):
    ...
    '''
    slapd.add_group('add_user', gid=50001, ldif_path=prefix + 'ldap-add-group.ldif')
    # by groupname
    slapd.add_user('user1', 'add_user', 'test1234', ldif_path=prefix + 'ldap-add-user.ldif', **config)
